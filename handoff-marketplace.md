...
**Original price, buyer side** — `ListingDetailPage.tsx` shows "Bought brand new at ₦X, save ₦Y" as a single plain `mkt-help` line under the title, understated by design (no colour, no tag, doesn't compete with the price above it). Chose to show the **saving amount**, not a percentage — a concrete naira figure reads faster than a percentage a buyer has to do the maths on themselves, and matches the app's naira-first tone throughout. Renders nothing at all when `original_price_naira` is null or not genuinely higher than `final_price_naira`. `LISTING_SELECT` (`data/mdb.ts`) and `MarketplaceListing` (`types.ts`) both extended with the new column. **Live-verified**: temporarily set a real listing's `original_price_naira` to ₦40,000 against its real ₦24,000 selling price, confirmed "Bought brand new at ₦40,000, save ₦16,000" rendered exactly, then confirmed a second real listing with no original price shows nothing, no empty state, before reverting the test value.
