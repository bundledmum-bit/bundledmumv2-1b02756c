...
export default function BundleSections({ variant = "shop" }: { variant?: Variant }) {
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        ...
        .select(`
          ...
          brands:brands_public!brands_product_id_fkey ( id, sku, brand_name, price, tier, in_stock, image_url, stored_image_url, images )
        `)
        .eq("is_gift_box", true)
        .eq("is_active", true)
  ...
  const MATERNITY_SECTION_CONFIG = { title: variant === "bundles" ? "Maternity + Baby Essentials" : "Bundles & Kits", subtitle: variant === "bundles" ? "Complete hospital bag and baby prep lists, curated by budget" : "Maternity + Baby Essential bundles for every budget", shopOrder: 30, bundlesOrder: 30, visible: true, items: maternityBundles, grid: variant === "shop" ? "1-2-4" as const : "1-2-3" as const, slug: "maternity-bundles", seeAllLabel: "See all Maternity + Baby Essentials" };
